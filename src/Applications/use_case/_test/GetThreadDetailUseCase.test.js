...
describe('GetThreadDetailUseCase', () => {
  it('should orchestrate get thread detail correctly including masking deleted content', async () => {
    ...
    // Raw data from repository (includes isDelete flag, no masking yet)
    const mockComments = [
    ...
    // Use case masks deleted content AND returns correct field order per Dicoding spec
      ...
      comments: [
        {
          ...
          replies: [
            // reply fields: id, content, date, username  (Dicoding spec)
          ...
          content: 'sebuah comment',        // not deleted → original
        },
        {
          id: 'comment-2',
          username: 'dicoding',
          ...
          replies: [],
          content: '**komentar telah dihapus**',  // deleted → masked
        },
      ],
    });
